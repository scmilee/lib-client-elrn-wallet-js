...
suite('seedToPublicKey', function() {
    test('seedToPublicKey returns a string', function() {
        ...
        return elrnClient.seedToPublicKey(seed, "m/44'/0'/0'/0/0", 'bitcoin')
        .then((address) => {
    ...
    test('litecoin public key', function() {
        ...
        return elrnClient.seedToPublicKey(seed, derivePath, 'litecoin')
        .then((publicKey) => {
            assert.equal(typeof publicKey,  'string')
            assert.equal(publicKey, '032545952b54ec0e4ee0a1b459e5d9a21745dc0d3408a83dd1638483c0ee55b41e')
        })
    });
    test('dogecoin public key', function() {
      // calcForDerivationPath()
      // https://github.com/iancoleman/bip39/blob/master/src/js/index.js
        const config = require(__dirname + '/config/options.js');
        const elrnClient = new Elrn(config)
        ...
        return elrnClient.seedToPublicKey(seed, derivePath, 'dogecoin')
        .then((publicKey) => {
            assert.equal(typeof publicKey,  'string')
            assert.equal(publicKey, '03db0a9330d8cf4248765b07b5106e5b75e005fcdffb9909c51ce145e3b6f5f7a5')
        })
    });
    // test('zcash public key', function() {
    //   // calcForDerivationPath()
    //   // https://github.com/iancoleman/bip39/blob/master/src/js/index.js
    //     const config = require(__dirname + '/config/options.js');
    //     const elrnClient = new Elrn(config)
