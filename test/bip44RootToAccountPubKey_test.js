...
suite('bip44RootToAccountPubKey', function() {
    test('returns a bitcoin account extended priv key', function() {
        ...
        elrnClient.seedToRoot(seed)
            .then((rootNode) => {
                return elrnClient.bip44RootToAccountPubKey(rootNode, derivedPathStr , accountNumber)
                .then((address) => {
    ...
    test('returns a ethereum account extended priv key', function() {
        ...
        elrnClient.seedToRoot(seed)
            .then((rootNode) => {
                return elrnClient.bip44RootToAccountPubKey(rootNode, derivedPathStr , accountNumber)
                .then((address) => {
                    assert.equal(address, 'xpub6C9rGmtBx5a2siNQ3hqQK94perhAhMSR7SYfjCcMXPcTCDYA3nxLcbtkbPtuL1j13Cu8gRExDZGbzVAf36G2YNP7Y6yXd9S7cDENWn48eff');
                })
            })
    });
});
