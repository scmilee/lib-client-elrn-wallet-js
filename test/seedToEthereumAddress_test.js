...
suite('seedToEthereumAddress', () => {
    ...
    test('Errors if no key is supplied', () => {
        return elrnClient.seedToEthereumAddress(null, path)
        .then((result) => assert.notEqual(true, true))
        .catch((error) => assert.equal(error.message, 'must call seedToEthereumAddress with a key'));
    });
    test('Errors if no path is supplied', () => {
        return elrnClient.seedToEthereumAddress(key, path)
        .then((result) => assert.notEqual(true, true))
        .catch((error) => assert.equal(error.message, 'must call seedToEthereumAddress with a path'));
    });
